...
var savedData = require('./savedData.json');
var fs = require('fs');
...
module.exports = {
    ...
    getMiddleware: function (options) {
        ...
        let saveRequests = options.saveRequests || false;
        let savedDataFile = __dirname + '/savedData.json';
        if (!saveRequests){
            let json = JSON.stringify({ requestsTotal: 0 });
            fs.writeFileSync(savedDataFile, json);
        }

        processor = new swsProcessor();
        processor.init(swsOptions);

        let timeForUpdateRequests = 60000; // 1min
        let dateUpdate = +new Date() + timeForUpdateRequests;

        return function trackingMiddleware(req, res, next) {

            if (dateUpdate < +new Date()){
                let json = JSON.stringify({ requestsTotal: processor.getStats().all.requestsTotal });
                fs.writeFileSync(savedDataFile, json);
                dateUpdate = + new Date() + timeForUpdateRequests;
            }
